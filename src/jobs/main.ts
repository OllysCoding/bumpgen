...
const channelTask = async (channel: XmltvChannel, programmes: XmltvProgramme[]): ReturnType<typeof makeVideo> => {
  ...
  return makeVideo({ 
    ...
    background: appConfig.backgroundContentFolder ? {
      filePath: path.join(appConfig.backgroundContentFolder, 'background-1.mp4'),
      startSeconds: 10,
      endSeconds: 2711
    } : undefined
  });
  ...
  if (isSuccess(xmlTv)) {
    for (const channel of xmlTv.result.channels) {
      ...
      if (isFailure(result)) {
        ...
        logInfo(`Completed task for channel ${channel.id} (Video ${result.result.replace('-', ' ')})`);
      }
    }
  }
};
