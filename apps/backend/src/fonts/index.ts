...
export abstract class Fonts {
  private static _fonts: Record<string, {
    family: string,
  ...
  static getFontProperties (family: string, weight?: string, style?: string) {
    ...
    const font = this._fonts[family]?.find((props) => props.weight === weightOrDefault &&  props.style === styleOrDefault);
    ...
    for (const file of allFiles) { 
      ...
      try {
        ...
        if (validate(parsed)) {
          ...
          for (const [fontFileName, properties] of Object.entries(parsed.files)) {
            ...
            try {
              ...
              logError(`Encountered errors when registering font at ${fontFilePath}`, err)
            }
          }
        } else { 
          logError(`Encountered errors when parsing font map at ${file}`, validate.errors);
         }
      } catch (err) {
